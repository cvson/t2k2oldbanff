{
   ...
   TFile *pfile_stat[NSAMPLE];
    ...
    TString subname = "t2kpre_dcp_point1_100k4check_100ksensi_wreactorthrow_optv2s13off_truedcp_lohi";
    //make graph
...
//fraction to sovle dcp
  ...
  double sigmaValue = 9.0;//3sigma
   for (j=0;j<Npointsearch[isam];j++) {
         ...
         //maximum
         //if (TMath::AreEqualRel(sigmaValue,pzerodcpExclusive30->Eval(tempx),/*0.1e-2*/0.1e-3)) {
         //  printf("found point %d with x=%g, y=%g\n",j,tempx,pzerodcpExclusive30->Eval(tempx));
        // }
        
       //if (TMath::AreEqualRel(sigmaValue,pzerodcpExclusive_stat->Eval(tempx),/*0.1e-2*/0.2e-3)) {
       //    printf("found point %d with x=%g, y=%g\n",j,tempx,pzerodcpExclusive_stat->Eval(tempx));
       //  }
