...
TFile *pfile_stat_inv[NSAMPLE];
    ...
    hisdcp30_inv[isam] = (TH1D*) pfile30_inv[isam]->Get("SA_dcp");
       ...
       pfile_stat[isam] = new TFile(Form("../plots/pac30e21_dcp_point1_100k4check_optv2s13off_sin23hi_100ksensi_dcp%d_wreactorthrow.root",isam+1));
    hisdcp_stat[isam] = (TH1D*) pfile_stat[isam]->Get("SA_dcp");
    
    pfile30_stat[isam] = new TFile(Form("../plots/pac30e21_dcp_point1_100k4check_optv2s13off_sin23lo_100ksensi_dcp%d_wreactorthrow.root",isam+1));
    hisdcp30_stat[isam] = (TH1D*) pfile30_stat[isam]->Get("SA_dcp");
   
pfile_stat_inv[isam] = new TFile(Form("../plots/pac30e21_dcp_point1_100k4check_optv2s13off_sin23hi_100ksensi_dcp%d_inv_wreactorthrow.root",isam+1));
    hisdcp_stat_inv[isam] = (TH1D*) pfile_stat_inv[isam]->Get("SA_dcp");
    
    pfile30_stat_inv[isam] = new TFile(Form("../plots/pac30e21_dcp_point1_100k4check_optv2s13off_sin23lo_100ksensi_dcp%d_inv_wreactorthrow.root",isam+1));
    hisdcp30_stat_inv[isam] = (TH1D*) pfile30_stat_inv[isam]->Get("SA_dcp");

 }
    
    TString subname = "t2kpre_dcp_point1_100k4check_100ksensi_wreactorthrow_optv2s13off_truedcp_unknownMH_lohi";
    //make graph
...
TLegend* leg1 = new TLegend(.17, .79, .85, .90);
    ...
    leg1->SetTextSize(18);
    leg1->SetTextFont(43);
leg1->AddEntry(s330,"20x10^{21} POT w/ eff. stat. improvements & 2016 sys. errs.","l");
