{
   ...
   TFile *pfile_stat[NSAMPLE];
    ...
    TString subname = "t2kpre_dcp_point1_100k4check_100ksensi_wreactorthrow_optv2s13off_truedcp_fakesyst";
    //make graph
...
//fraction to sovle dcp
  ...
  //double sigmaValue = 9.0;//3sigma
   for (j=0;j<Npointsearch[isam];j++) {
         ...
         if (TMath::AreEqualRel(sigmaValue,pzerodcpExclusive30->Eval(tempx),/*0.1e-2*/0.2e-3)) {
