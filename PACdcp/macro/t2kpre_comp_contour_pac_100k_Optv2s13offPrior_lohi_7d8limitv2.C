{
    ...
    const char *m_name[NSAMPLE] = {"current","pac2d5e21","pac5e21","pac6e21","pac7d8e21"};
    const char *m_name_long[NSAMPLE] = {"current","2.5e21","5e21","6e21","7.8e21"};
    TFile *pfile[NSAMPLE];
    ...
    for (Int_t isam=0; isam<NSAMPLE; ++isam) {
        ...
        pfilesystB[isam] = new TFile(Form("../plots/%s_dcp_point1_100k4check_optv2s13off_sin23hi_100ksensi_wreactorthrow.root",m_name[isam]));
        pfilesystC[isam] = new TFile(Form("../plots/%s_dcp_point1_100k4check_optv2s13off_sin23lo_100ksensi_wreactorthrow.root",m_name[isam]));
    ...
    TString subname = "t2kpre_dcp_point1_100k_wreactorthrow_all_optv2s13off_statvssyst_lohi_7d8limitv2";
    cout<<"Number of entries "<<pchain->GetEntries()<<" should equal "<<NSAMPLE<<endl;
    //make graph
   Double_t xpot[NSAMPLE]={0,2.5,5,6,7.8};
...
s3B->SetLineWidth(3);
s3C->SetLineWidth(3);
pzerodcpExclusive->SetMaximum(5);
pzerodcpExclusive->SetLineStyle(1);
pzerodcpExclusiveB->SetLineStyle(1);
pzerodcpExclusiveC->SetLineStyle(1);
pzerodcpExclusive->SetLineColor(kWhite);
pzerodcpExclusive->Draw("AL");
pzerodcpExclusive->GetXaxis()->SetRangeUser(0,7.8);
gPad->Modified();
//pzerodcpExclusiveB->Draw("L same");
//pzerodcpExclusiveC->Draw("L same");
pzerodcpExclusivesyst->SetLineStyle(7);
pzerodcpExclusivesystB->SetLineStyle(7);
pzerodcpExclusivesystC->SetLineStyle(7);

//pzerodcpExclusivesyst->Draw("L same");
//pzerodcpExclusivesystB->Draw("L same");
//pzerodcpExclusivesystC->Draw("L same");
s3->Draw("L same");
s3B->Draw("L same");
s3C->Draw("L same");
s3syst->Draw("L same");
...
leg0->Draw();
   ci = TColor::GetColor("#000000");
    ...
    tex = new TLatex(.4,levels[0]+0.1,"  90% C.L.");
    tex->SetTextAlign(11);
    tex->SetTextFont(43);
    tex->SetTextColor(ci);
    tex->SetTextSize(15);
 ...
 ci = TColor::GetColor("#000000");
    ...
    leg1->SetTextSize(18);
    leg1->SetTextFont(43);
leg1->AddEntry(s3,"w/ no systematic error","l");
leg1->AddEntry(s3syst,"w/ 2016 systematic error","l");
leg1->Draw();
