...
s3syst->Draw("L same");
s3systB->Draw("L same");
TLegend* leg0 = new TLegend(.25, .65, 0.85, .87);
leg0->SetMargin(0.15);
    leg0->SetFillStyle(0);
    leg0->SetBorderSize(0);
    leg0->SetTextSize(18);
    leg0->SetTextFont(43);
leg0->AddEntry(s3,"w/ eff. stat. improvement (no sys. errors)","l");
leg0->AddEntry(s3syst,"w/ 2016 sys. errors","l");
leg0->AddEntry(s3systB,"w/ improved sys. errors","l");
leg0->Draw();
 double ypos = 0.96;
    double xpos =0.75;
    TLatex *   text2kpre = new TLatex(xpos,ypos,"Work in Progress");//remove PRELIMINARY
