...
"#D55E00",
    ...
    for (Int_t isam=0; isam<NSAMPLE; ++isam) {
        ...
        pfile1[isam][ithrow] = new TFile(Form("../plots/%s_dcp_point1_optv2s13off_%sposcpinv_inv_100ksensi_wreactorthrow_StatOnly.root",m_throw1[ithrow],m_name[isam]));
    hisdcp1[isam][ithrow] = (TH1D*) pfile1[isam][ithrow]->Get("SA_dcp");

       pfile2[isam][ithrow] = new TFile(Form("../plots/%s_dcp_point1_optv2s13off_%sposcpinv_inv_100ksensi_wreactorthrow_StatOnly.root",m_throw2[ithrow],m_name[isam]));
    ...
    TString subname = "t2kpre_dcp_point1_wreactorthrow_StatOnly_allExclusive_mod2_inv";
    ...
    graphdcpExclu[isam][4]->GetXaxis()->SetRangeUser(0,30);
    ...
    tex[2]->Draw();
    leg0[isam]->Draw();
    TLatex *texsn = new TLatex(16,18,m_name_long[isam]);
