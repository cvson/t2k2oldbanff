...
$script = "./runsensiAsimov_5e21_wreactorthrow_dcp_StatOnly.sh";
#$script = "./runsensiAsimov_5e21_wreactorthrow_dcp_inverted_StatOnly.sh";
#$script = "./runsensiAsimov_5e21_wreactorthrow_dcp.sh";
#$script = "./runsensiAsimov_5e21_wreactorthrow_dcp_inverted.sh";
...
for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
  ...
  system("echo $run_num");
  system("/bin/rm -f run/job_asimov_5e21_$run_num.sh");

##
  open(OUT,">run/job_asimov_5e21_$run_num.sh");
  print(OUT "\#\!/bin/sh -f\n");
  #print(OUT "\#BSUB -q e\n");
  print(OUT "\#BSUB -q s\n");
  print(OUT "\#BSUB -J \"job_asimov_5e21_$run_num\"\n");
  print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdcp/shell\n");
  print(OUT "$script $run_num");
  close(OUT);

  system("chmod a+x run/job_asimov_5e21_$run_num.sh");
    # system("bsub < run/job_asimov_5e21_$run_num.sh");
   #system("/bin/sh run/job_asimov_5e21_$run_num.sh");
    #system("/bin/rm -f run/job_asimov_5e21_$run_num.sh");
...
open(OUT,">run/job_asimov_5e21_all.sh");
    print(OUT "\#\!/bin/sh -f\n");
    print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdcp/shell/run\n");
    for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
        print(OUT "./job_asimov_5e21_$run_num.sh\n");
        select(undef, undef, undef, 0.05);
    }
    close(OUT);
    system("chmod a+x run/job_asimov_5e21_all.sh");
    system("bsub < run/job_asimov_5e21_all.sh");
#$start_merge = 0;
#    $end_merge = 4;
#    for($run_merge = $start_merge; $run_merge <= $end_merge; $run_merge++) {
#        open(OUT,">run/job_asimov_5e21_merge_$run_merge.sh");
#        print(OUT "\#\!/bin/sh -f\n");
#        print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdcp/shell/run\n");
#        for($run_num = 1; $run_num <= 20; $run_num++) {
#            $mergeindex = $run_num + $run_merge *20;
#            print(OUT "./job_asimov_5e21_$mergeindex.sh\n");
#            select(undef, undef, undef, 0.05);
#        }
#        if($run_merge ==4) {
#            print(OUT "./job_asimov_5e21_101.sh\n");
#        }
#        close(OUT);
#        system("chmod a+x run/job_asimov_5e21_merge_$run_merge.sh");
#        system("bsub < run/job_asimov_5e21_merge_$run_merge.sh");
#    }
