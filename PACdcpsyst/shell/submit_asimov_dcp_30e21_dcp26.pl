...
$script = "./runsensiAsimov_30e21_wreactorthrow_dcp26_inverted.sh";
...
for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
  ...
  system("echo $run_num");
  system("/bin/rm -f run/job_asimov_30e21_dcp26_$run_num.sh");

##
  open(OUT,">run/job_asimov_30e21_dcp26_$run_num.sh");
  print(OUT "\#\!/bin/sh -f\n");
  #print(OUT "\#BSUB -q e\n");
  print(OUT "\#BSUB -q s\n");
  print(OUT "\#BSUB -J \"job_asimov_30e21_dcp26_$run_num\"\n");
  print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdcpsyst/shell\n");
  print(OUT "$script $run_num");
  close(OUT);

  system("chmod a+x run/job_asimov_30e21_dcp26_$run_num.sh");
    # system("bsub < run/job_asimov_30e21_dcp26_$run_num.sh");
    #system("/bin/sh run/job_asimov_30e21_dcp26_$run_num.sh");
    #system("/bin/rm -f run/job_asimov_30e21_dcp26_$run_num.sh");
...
open(OUT,">run/job_asimov_30e21_dcp26_all.sh");
print(OUT "\#\!/bin/sh -f\n");
print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdcpsyst/shell/run\n");
for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
print(OUT "./job_asimov_30e21_dcp26_$run_num.sh\n");
select(undef, undef, undef, 0.05);
}
close(OUT);
system("chmod a+x run/job_asimov_30e21_dcp26_all.sh");
system("bsub < run/job_asimov_30e21_dcp26_all.sh");
