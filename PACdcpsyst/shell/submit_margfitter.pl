...
$script = "./runmargfitter_run16fix_StatOnly.sh";
#$end_run = 4131;
...
for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
  ...
  system("echo $run_num");
  system("/bin/rm -f run/job_margfitter_$run_num.sh");

##
  open(OUT,">run/job_margfitter_$run_num.sh");
  print(OUT "\#\!/bin/sh -f\n");
  #print(OUT "\#BSUB -q e\n");
  print(OUT "\#BSUB -q s\n");
  print(OUT "\#BSUB -J \"job_margfitter_$run_num\"\n");
  print(OUT "cd ~/gpfs/ptheta/pac2016/Ptheta_MultiSample/frJame/Numudis/shell\n");
  print(OUT "$script $run_num");
  close(OUT);

  system("chmod a+x run/job_margfitter_$run_num.sh");
     system("bsub < run/job_margfitter_$run_num.sh");
    #system("/bin/rm -f run/job_margfitter_$run_num.sh");
