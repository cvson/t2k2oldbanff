...
//Main Macro
void PlotSensiAsimov_dm32_reactor_test(TString SensiAsimovFile, TString filename){
    ...
    Sensi->Add("/home/t2k/cvson/ptheta/pac2016/20151128/Ptheta_MultiSample/PACdm32/outputs/SensiAsimov/"+SensiAsimovFile);
    ...
    TH1D *SA_dm32 = new TH1D("SA_dm32", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    
    TH1D *SA_norm[4];
    SA_norm[0] = new TH1D("SA_dm32_norm_nue", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_norm[1] = new TH1D("SA_dm32_norm_numu", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_norm[2] = new TH1D("SA_dm32_norm_nuebar", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_norm[3] = new TH1D("SA_dm32_norm_numubar", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    
    TH1D *SA_like[4];
    SA_like[0] = new TH1D("SA_dm32_Like_nue", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_like[1] = new TH1D("SA_dm32_Like_numu", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_like[2] = new TH1D("SA_dm32_Like_nuebar", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    SA_like[3] = new TH1D("SA_dm32_Like_numubar", "Asimov Sensitivity", 101, 1.975e-3, 3.025e-3);
    ...
    for(int i=0; i<Sensi->GetEntries(); i++){
        ...
        if(!TMath::Finite(AvNLLtot) || TMath::IsNaN(AvNLLtot) ){//newadd
            SA_dm32->SetBinContent(i+1, 1e4);
        }
        else SA_dm32->SetBinContent(i+1, 2*AvNLLtot - globalmin);
        for(int k=0; k<4; k++){
    ...
    best_all->SetPoint(0, SA_dm32->GetXaxis()->GetBinCenter(globalmin_bin+1), 0.);
