...
//Main Macro
void PlotSepFitter_appearance_test(TString SepFitterFile, TString filename){
  ...
  TH2D *DF_t13_dcp = new TH2D("DF_t13_dcp", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);

  TH2D *DF_norm[4];
  DF_norm[0] = new TH2D("DF_t13_dcp_norm_nue", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_norm[1] = new TH2D("DF_t13_dcp_norm_numu", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_norm[2] = new TH2D("DF_t13_dcp_norm_nuebar", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_norm[3] = new TH2D("DF_t13_dcp_norm_numubar", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);

  TH2D *DF_like[4];
  DF_like[0] = new TH2D("DF_t13_dcp_Like_nue", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_like[1] = new TH2D("DF_t13_dcp_Like_numu", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_like[2] = new TH2D("DF_t13_dcp_Like_nuebar", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  DF_like[3] = new TH2D("DF_t13_dcp_Like_numubar", "Data Fit", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  ...
  TFile *outFile = new TFile("../plots/SepFitter/"+filename+".root", "RECREATE");
  outFile->cd();
  ...
  for(int i=0; i<SepFit->GetEntries(); i++){
    ...
    for(int k=0; k<4; k++){
      if(NLL_norm[k] < norm_min[k]){
        norm_min[k] = NLL_norm[k];
	norm_min_bin[k] = i;
      }
      if(NLL_like[k] < like_min[k]){
        like_min[k] = NLL_like[k];
        like_min_bin[k] = i;
      }
    }
  }
  ...
  for(int i=0; i<SepFit->GetEntries(); i++){
    ...
    DF_t13_dcp->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, NLLtot - globalmin);
    for(int k=0; k<4; k++){
      //single sample shape only for sensitivity/data, Poisson likelihood for Asimov
      DF_like[k]->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, NLL_like[k] - like_min[k]);
      //single sample norm only
      DF_norm[k]->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, NLL_norm[k] - norm_min[k]);
    }
  ...
  best_all->SetPoint(0, DF_t13_dcp->GetXaxis()->GetBinCenter(TMath::FloorNint(globalmin_bin/51)+1), 
		     DF_t13_dcp->GetYaxis()->GetBinCenter((globalmin_bin%51)+1));
  best_all_conv->SetPoint(0, 0.5*(1-TMath::Sqrt(1-DF_t13_dcp->GetXaxis()->GetBinCenter(TMath::FloorNint(globalmin_bin/51)+1))), 
		     DF_t13_dcp->GetYaxis()->GetBinCenter((globalmin_bin%51)+1));
  ...
  for(int k=0; k<4; k++){
    ...
    best_norm[k]->SetPoint(0, DF_norm[k]->GetXaxis()->GetBinCenter(TMath::FloorNint(norm_min_bin[k]/51)+1), 
		       DF_norm[k]->GetYaxis()->GetBinCenter((norm_min_bin[k]%51)+1));
    PrepareBestPoint(best_norm[k]);
    best_norm[k]->Write("BestFit_Point_norm_"+gr_names[k]);

    best_like[k] = new TGraph(1);
    best_like[k]->SetPoint(0, DF_like[k]->GetXaxis()->GetBinCenter(TMath::FloorNint(like_min_bin[k]/51)+1), 
		       DF_like[k]->GetYaxis()->GetBinCenter((like_min_bin[k]%51)+1));
    PrepareBestPoint(best_like[k]);
    best_like[k]->Write("BestFit_Point_like_"+gr_names[k]);

    best_norm_conv[k] = new TGraph(1);
    best_norm_conv[k]->SetPoint(0, 0.5*(1-TMath::Sqrt(1-DF_norm[k]->GetXaxis()->GetBinCenter(TMath::FloorNint(norm_min_bin[k]/51)+1))), 
		       DF_norm[k]->GetYaxis()->GetBinCenter((norm_min_bin[k]%51)+1));
    PrepareBestPoint(best_norm_conv[k]);
    best_norm_conv[k]->Write("BestFit_Point_norm_conv_"+gr_names[k]);

    best_like_conv[k] = new TGraph(1);
    best_like_conv[k]->SetPoint(0, 0.5*(1-TMath::Sqrt(1-DF_like[k]->GetXaxis()->GetBinCenter(TMath::FloorNint(like_min_bin[k]/51)+1))), 
		       DF_like[k]->GetYaxis()->GetBinCenter((like_min_bin[k]%51)+1));
  ...
  c2->cd();
  DF_t13_dcp->Draw("colz");
  TH2D* DF_t13_dcp_cont = (TH2D*)DF_t13_dcp->Clone();
  DF_t13_dcp_cont->SetContour(nlevels, levels);
  DF_t13_dcp_cont->Draw("cont list");
  c2->Update();
  ...
  for(int i=0; i<4; i++){
    c3->cd(i+1);
    DF_norm[i]->Draw("colz");
    DF_norm_cont[i] = (TH2D*)DF_norm[i]->Clone();
    DF_norm_cont[i]->SetContour(nlevels, levels);
    DF_norm_cont[i]->Draw("cont list");
    c3->Update();
  ...
  for(int i=0; i<4; i++){
    c4->cd(i+1);
    DF_like[i]->Draw("colz");
    DF_like_cont[i] = (TH2D*)DF_like[i]->Clone();
    DF_like_cont[i]->SetContour(nlevels, levels);
    DF_like_cont[i]->Draw("cont list");
    c4->Update();
...
//#############################################################################
void PrintHisto(TCanvas *canvas, TString name, TString filename){
  canvas->Print("../plots/SepFitter/"+filename+"/"+name+".eps");
  canvas->Print("../plots/SepFitter/"+filename+"/"+name+".pdf");
  canvas->Print("../plots/SepFitter/"+filename+"/"+name+".png");
  canvas->Print("../plots/SepFitter/"+filename+"/"+name+".C");
}
//#############################################################################
std::vector<TGraph*> GetContourGraphs(TObjArray *contours, TString color, int twotheta, int graphNum){
