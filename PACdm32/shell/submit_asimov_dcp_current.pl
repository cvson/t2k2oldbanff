...
# system("/bin/sh run/job_asimov_$run_num.sh");  
 ...
 $start_merge = 0;
    ...
    for($run_merge = $start_merge; $run_merge <= $end_merge; $run_merge++) {
        open(OUT,">run/job_asimov_merge_$run_merge.sh");
        print(OUT "\#\!/bin/sh -f\n");
        ...
        for($run_num = 1; $run_num <= 20; $run_num++) {
            ...
            print(OUT "./job_asimov_$mergeindex.sh\n");
            select(undef, undef, undef, 0.05);
        }
        if($run_merge ==4) {
            print(OUT "./job_asimov_101.sh\n");
        }
        close(OUT);
        system("chmod a+x run/job_asimov_merge_$run_merge.sh");
        system("bsub < run/job_asimov_merge_$run_merge.sh");
}
