...
//Main Macro
void PlotSensiAsimov_appearance_raster_test(TString SensiAsimovFile, TString filename){
  ...
  TH2D *SA_t13_dcp = new TH2D("SA_t13_dcp", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);

  TH2D *SA_norm[4];
  SA_norm[0] = new TH2D("SA_t13_dcp_norm_nue", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_norm[1] = new TH2D("SA_t13_dcp_norm_numu", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_norm[2] = new TH2D("SA_t13_dcp_norm_nuebar", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_norm[3] = new TH2D("SA_t13_dcp_norm_numubar", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);

  TH2D *SA_like[4];
  SA_like[0] = new TH2D("SA_t13_dcp_Like_nue", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_like[1] = new TH2D("SA_t13_dcp_Like_numu", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_like[2] = new TH2D("SA_t13_dcp_Like_nuebar", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  SA_like[3] = new TH2D("SA_t13_dcp_Like_numubar", "Asimov Sensitivity 1D Raster", 81, -0.005, 0.805, 51, -3.2028, 3.2028);
  ...
  TFile *outFile = new TFile("../plots/"+filename+"_raster.root", "RECREATE");
  outFile->cd();

  for(int i=0; i<Sensi->GetEntries(); i++){
    Sensi->GetEntry(i);

    if(AvNLLtot < globalmin[i%51]){
      globalmin[i%51] = AvNLLtot;
      globalmin_bin[i%51] = TMath::FloorNint(i/51);
    }

    for(int k=0; k<4; k++){
      if(AvNLL_norm[k] < norm_min[k][i%51]){
        norm_min[k][i%51] = AvNLL_norm[k];
	norm_min_bin[k][i%51] = TMath::FloorNint(i/51);
      }
      if(AvNLL_like[k] < like_min[k][i%51]){
        like_min[k][i%51] = AvNLL_like[k];
  ...
  for(int i=0; i<Sensi->GetEntries(); i++){
    ...
    SA_t13_dcp->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, AvNLLtot - globalmin[i%51]);
    for(int k=0; k<4; k++){
      //single sample shape only for Sensitivity, Poisson likelihood for Asimov
      SA_like[k]->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, AvNLL_like[k] - like_min[k][i%51]);
      //single sample norm only
      SA_norm[k]->SetBinContent(TMath::FloorNint(i/51)+1, (i%51)+1, AvNLL_norm[k] - norm_min[k][i%51]);
    }
...
//#############################################################################
void PrintHisto(TCanvas *canvas, TString name, TString filename){
  canvas->Print("../plots/"+filename+"_raster/"+name+".eps");
  canvas->Print("../plots/"+filename+"_raster/"+name+".pdf");
  canvas->Print("../plots/"+filename+"_raster/"+name+".png");
  canvas->Print("../plots/"+filename+"_raster/"+name+".C");
}
//#############################################################################
std::vector<TGraph*> GetContourGraphs(TObjArray *contours, TString color, int twotheta, int graphNum){
