...
//Main Macro
void PlotSensiAsimov_sin23_reactor_test(TString SensiAsimovFile, TString filename){
    ...
    Sensi->Add("/home/t2k/cvson/ptheta/pac2016/20151128/Ptheta_MultiSample/PACsin23/outputs/SensiAsimov/"+SensiAsimovFile);
    ...
    TH1D *SA_sin23 = new TH1D("SA_sin23", "Asimov Sensitivity", 51, 0.296, 0.704);
    
    TH1D *SA_norm[4];
    SA_norm[0] = new TH1D("SA_sin23_norm_nue", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_norm[1] = new TH1D("SA_sin23_norm_numu", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_norm[2] = new TH1D("SA_sin23_norm_nuebar", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_norm[3] = new TH1D("SA_sin23_norm_numubar", "Asimov Sensitivity", 51, 0.296, 0.704);
    
    TH1D *SA_like[4];
    SA_like[0] = new TH1D("SA_sin23_Like_nue", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_like[1] = new TH1D("SA_sin23_Like_numu", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_like[2] = new TH1D("SA_sin23_Like_nuebar", "Asimov Sensitivity", 51, 0.296, 0.704);
    SA_like[3] = new TH1D("SA_sin23_Like_numubar", "Asimov Sensitivity", 51, 0.296, 0.704);
    ...
    for(int i=0; i<Sensi->GetEntries(); i++){
        ...
        if(!TMath::Finite(AvNLLtot) || TMath::IsNaN(AvNLLtot) ){//newadd
            SA_sin23->SetBinContent(i+1, 1e4);
        }
        else SA_sin23->SetBinContent(i+1, 2*AvNLLtot - globalmin);
        for(int k=0; k<4; k++){
            //single sample shape only for Sensitivity, Poisson likelihood for Asimov
            if(!TMath::Finite(AvNLL_like[k]) || TMath::IsNaN(AvNLL_like[k]) ){
                SA_like[k]->SetBinContent(i+1, 1e4);
            }
            else SA_like[k]->SetBinContent(i+1, 2*AvNLL_like[k] - like_min[k]);
            //single sample norm only
            if(!TMath::Finite(AvNLL_norm[k]) || TMath::IsNaN(AvNLL_norm[k]) ) {
                SA_norm[k]->SetBinContent(i+1, 1e4);
            }
            else SA_norm[k]->SetBinContent(i+1, 2*AvNLL_norm[k] - norm_min[k]);
        }
    ...
    best_all->SetPoint(0, SA_sin23->GetXaxis()->GetBinCenter(globalmin_bin+1), 0.);
...
//#############################################################################
void PreparePlot(TH1D* plot){
    plot->GetXaxis()->SetTitle("#Delta m^2_{32}");
