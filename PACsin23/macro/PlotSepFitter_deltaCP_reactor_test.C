...
//Main Macro
void PlotSepFitter_deltaCP_reactor_test(TString SepFitterFile, TString filename){
  ...
  TH1D *DF_dcp = new TH1D("DF_dcp", "Data Fit", 51, -3.2028, 3.2028);

  TH1D *DF_norm[4];
  DF_norm[0] = new TH1D("DF_dcp_norm_nue", "Data Fit", 51, -3.2028, 3.2028);
  DF_norm[1] = new TH1D("DF_dcp_norm_numu", "Data Fit", 51, -3.2028, 3.2028);
  DF_norm[2] = new TH1D("DF_dcp_norm_nuebar", "Data Fit", 51, -3.2028, 3.2028);
  DF_norm[3] = new TH1D("DF_dcp_norm_numubar", "Data Fit", 51, -3.2028, 3.2028);

  TH1D *DF_like[4];
  DF_like[0] = new TH1D("DF_dcp_Like_nue", "Data Fit", 51, -3.2028, 3.2028);
  DF_like[1] = new TH1D("DF_dcp_Like_numu", "Data Fit", 51, -3.2028, 3.2028);
  DF_like[2] = new TH1D("DF_dcp_Like_nuebar", "Data Fit", 51, -3.2028, 3.2028);
  DF_like[3] = new TH1D("DF_dcp_Like_numubar", "Data Fit", 51, -3.2028, 3.2028);
  ...
  outFile->cd();
  TTree *Minima = new TTree("MinimaTree", "tree");
  Minima->Branch("globalmin", &globalmin, "globalmin/D");
  Minima->Branch("norm_min", &norm_min, "norm_min[4]/D");
  Minima->Branch("like_min", &like_min, "like_min[4]/D");
  Minima->Branch("globalmin_bin", &globalmin_bin, "globalmin_bin/I");
  Minima->Branch("norm_min_bin", &norm_min_bin, "norm_min_bin[4]/I");
  Minima->Branch("like_min_bin", &like_min_bin, "like_min_bin[4]/I");


  for(int i=0; i<SepFit->GetEntries(); i++){
    SepFit->GetEntry(i);

    if(2*NLLtot < globalmin){
      globalmin = 2*NLLtot;
      globalmin_bin = i;
    }

    for(int k=0; k<4; k++){
      if(2*NLL_norm[k] < norm_min[k]){
        norm_min[k] = 2*NLL_norm[k];
	norm_min_bin[k] = i;
      }
      if(2*NLL_like[k] < like_min[k]){
        like_min[k] = 2*NLL_like[k];
        like_min_bin[k] = i;
      }
    }
  }
  ...
  for(int i=0; i<SepFit->GetEntries(); i++){
    ...
    DF_dcp->SetBinContent(i+1, 2*NLLtot - globalmin);
    for(int k=0; k<4; k++){
      //single sample shape only for Sensitivity, Poisson likelihood for Asimov
      DF_like[k]->SetBinContent(i+1, 2*NLL_like[k] - like_min[k]);
      //single sample norm only
      DF_norm[k]->SetBinContent(i+1, 2*NLL_norm[k] - norm_min[k]);
    }
  ...
  best_all->SetPoint(0, DF_dcp->GetXaxis()->GetBinCenter(globalmin_bin+1), 0.);
  ...
  for(int k=0; k<4; k++){
    ...
    best_norm[k]->SetPoint(0, DF_norm[k]->GetXaxis()->GetBinCenter(norm_min_bin[k]+1), 0.);
    PrepareBestPoint(best_norm[k]);
    best_norm[k]->Write("BestFit_Point_norm_"+gr_names[k]);

    best_like[k] = new TGraph(1);
    best_like[k]->SetPoint(0, DF_like[k]->GetXaxis()->GetBinCenter(like_min_bin[k]+1), 0.);
