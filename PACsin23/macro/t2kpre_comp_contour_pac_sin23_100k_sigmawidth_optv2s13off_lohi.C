...
TFile *pfile_hi[NSAMPLE];
    ...
    for (Int_t isam=0; isam<NSAMPLE; ++isam) {
        if(isam==0){pfile[isam] = new TFile(Form("../plots/%s_s13off_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
         pfilesyst[isam] = new TFile(Form("../plots/%s_s13off_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
        pfile_lo[isam] = new TFile(Form("../plots/%s_s13off_sin23lo_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
    pfilesyst_lo[isam] = new TFile(Form("../plots/%s_s13off_sin23lo_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
    
    pfile_hi[isam] = new TFile(Form("../plots/%s_s13off_sin23hi_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
    pfilesyst_hi[isam] = new TFile(Form("../plots/%s_s13off_sin23hi_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam])); 
        
         }
        else {pfile[isam] = new TFile(Form("../plots/%s_optv2s13off_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
         pfilesyst[isam] = new TFile(Form("../plots/%s_optv2s13off_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));

         pfile_lo[isam] = new TFile(Form("../plots/%s_optv2s13off_sin23lo_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
    pfilesyst_lo[isam] = new TFile(Form("../plots/%s_optv2s13off_sin23lo_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
    
    pfile_hi[isam] = new TFile(Form("../plots/%s_optv2s13off_sin23hi_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
    pfilesyst_hi[isam] = new TFile(Form("../plots/%s_optv2s13off_sin23hi_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
         }
        hisdcp[isam] = (TH1D*) pfile[isam]->Get("SA_sin23");
        hisdcpsyst[isam] = (TH1D*) pfilesyst[isam]->Get("SA_sin23");
       ...
       hisdcp_lo[isam] = (TH1D*) pfile_lo[isam]->Get("SA_sin23");
    hisdcpsyst_lo[isam] = (TH1D*) pfilesyst_lo[isam]->Get("SA_sin23");
...
hisdcp_hi[isam] = (TH1D*) pfile_hi[isam]->Get("SA_sin23");
    hisdcpsyst_hi[isam] = (TH1D*) pfilesyst_hi[isam]->Get("SA_sin23");
    ...
    hisdcpsyst_hi[isam]->SetLineWidth(3);
        if(isam==0){pchain->Add(Form("../plots/%s_s13off_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
        pchainsys->Add(Form("../plots/%s_s13off_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
      }
      else{pchain->Add(Form("../plots/%s_optv2s13off_wreactorthrow_StatOnly_sensiAsimov_100k_sin23.root",m_name[isam]));
        pchainsys->Add(Form("../plots/%s_optv2s13off_wreactorthrow_sensiAsimov_100k_sin23.root",m_name[isam]));
       }
 }
    
    //TString subname = "pac_dcp_point1_wreactorthrow";
    TString subname = "t2kpre_sin23_point1_wreactorthrow_all_100k_optv2s13off_lohi";
    ...
    //make graph
    double deviation[NSAMPLE]={1e-5,1e-5,1e-5,1e-5,1e-5};
   Int_t Npointsearch[NSAMPLE]={30000,30000,30000,30000,30000};
   ...
   Double_t yvalsyst_hi[NSAMPLE];
    for (Int_t isam=0; isam<NSAMPLE; isam++) {
        ...
        for (j=0;j<Npointsearch[isam];j++) {
         double tempx = 0.4+deviation[isam]*j;
         if (TMath::AreEqualRel(1,graphdcp[isam]->Eval(tempx),0.2e-2)) {
           printf("found point %d with x=%g, y=%g\n",j,tempx,graphdcp[isam]->Eval(tempx));
           if(tempx<0.5) tempxlow =tempx;
           else tempxhigh=tempx;
         }
        }
        ...
        for (j=0;j<Npointsearch[isam];j++) {
         double tempx = 0.4+deviation[isam]*j;
         if (TMath::AreEqualRel(1,graphdcpsyst[isam]->Eval(tempx),0.2e-2)) {
        ...
        yval_hi[isam]=0;
    yvalsyst_hi[isam]=0;
cout<<"PROCESSING HI"<<endl;
    cout<<"SAM "<<isam<<endl;
    graphdcp_hi[isam] = new TGraph(hisdcp_hi[isam]);
    for (j=0;j<Npointsearch[isam];j++) {
        double tempx = 0.5+deviation[isam]*j;
        if (TMath::AreEqualRel(1,graphdcp_hi[isam]->Eval(tempx),0.2e-2)) {
            printf("found point %d with x=%g, y=%g\n",j,tempx,graphdcp_hi[isam]->Eval(tempx));
            if(tempx<0.6) tempxlow =tempx;
            else if(tempx>0.5) tempxhigh=tempx;
        }
    }
    ...
    for (j=0;j<Npointsearch[isam];j++) {
        double tempx = 0.5+deviation[isam]*j;
        if (TMath::AreEqualRel(1,graphdcpsyst_hi[isam]->Eval(tempx),0.2e-2)) {
...
cout<<"PROCESSING LO"<<endl;
yval_lo[isam]=0;
    yvalsyst_lo[isam]=0;
    cout<<"SAM "<<isam<<endl;
    graphdcp_lo[isam] = new TGraph(hisdcp_lo[isam]);
    for (j=0;j<Npointsearch[isam];j++) {
        double tempx = 0.35+deviation[isam]*j;
        if (TMath::AreEqualRel(1,graphdcp_lo[isam]->Eval(tempx),0.2e-2)) {
            printf("found point %d with x=%g, y=%g\n",j,tempx,graphdcp_lo[isam]->Eval(tempx));
            if(tempx<0.43) tempxlow =tempx;
            else if(tempx<0.5) tempxhigh=tempx;
        }
    }
    yval_lo[isam] = tempxhigh - tempxlow;
   cout<<"TEMP LO "<<tempxhigh<<" LO "<<tempxlow<<" width "<<yval_lo[isam]<<endl; 
    graphdcpsyst_lo[isam] = new TGraph(hisdcpsyst_lo[isam]);
    for (j=0;j<Npointsearch[isam];j++) {
        double tempx = 0.35+deviation[isam]*j;
        if (TMath::AreEqualRel(1,graphdcpsyst_lo[isam]->Eval(tempx),0.2e-2)) {
...
cout<<"TEMP SYST LO "<<tempxhigh<<" LO "<<tempxlow<<" width "<<yvalsyst_lo[isam]<<endl;
        yval[isam] /=(1.e-2);//2.51e-3 devide by 2
        yvalsyst[isam] /=(1.e-2);
        yval_hi[isam] /=(1.2e-2);//0.5 times 2
        yvalsyst_hi[isam] /=(1.2e-2);//0.5 times 2 times 100 percentage
       yval_lo[isam] /=(0.86e-2);//0.5 times 2
        yvalsyst_lo[isam] /=(0.86e-2);//0.5 times 2 times 100 percentage
        cout<<"SAM "<<isam<<" width stat.  "<<yval[isam]<<" syst. "<<yvalsyst[isam]<<endl;
       cout<<"SAM "<<isam<<" LI width stat.  "<<yval_hi[isam]<<" syst. "<<yvalsyst_hi[isam]<<endl;
...
TGraph *dm32width_hi = new TGraph(NSAMPLE,xpot,yval_hi);
    ...
    dm32width->GetYaxis()->SetTitle("Uncertainty on sin^{2}#theta_{23} (%)");
    ...
    leg0->AddEntry(dm32width,"sin^{2}#theta_{23}=0.50","l");
    leg0->AddEntry(dm32width_hi,"sin^{2}#theta_{23}=0.60","l");
    leg0->Draw();
    ...
    /*new TCanvas;
    titleStyle(hisdcp[4]);
    hisdcp[4]->GetXaxis()->SetTitle("Sin^{2}#theta_{23}");
    hisdcp[4]->SetMinimum(0);
    hisdcp[4]->GetXaxis()->SetRangeUser(0.45,0.57);
    hisdcp[4]->Draw("C");
