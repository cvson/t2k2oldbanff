...
###for dcp
#$script = "./runmerger_7d8e21_wreactorthrow_dcp_StatOnly.sh";
#$script = "./runmerger_7d8e21_wreactorthrow_dcp_inverted_StatOnly.sh";
$script = "./runmerger_7d8e21_wreactorthrow_dcp.sh";
#$script = "./runmerger_7d8e21_wreactorthrow_dcp_inverted.sh";
...
for($run_num = $start_run; $run_num <= $end_run; $run_num++) {
  ...
  system("echo $run_num");
  system("/bin/rm -f run/job_merger_7d8e21_$run_num.sh");

##
  open(OUT,">run/job_merger_7d8e21_$run_num.sh");
  print(OUT "\#\!/bin/sh -f\n");
  #print(OUT "\#BSUB -q e\n");
  print(OUT "\#BSUB -q s\n");
  print(OUT "\#BSUB -J \"job_merger_7d8e21_$run_num\"\n");
  print(OUT "cd ~/gpfs/ptheta/pac2016/20151128/Ptheta_MultiSample/PACsin23/shell\n");
  print(OUT "$script $run_num");
  close(OUT);

  system("chmod a+x run/job_merger_7d8e21_$run_num.sh");
     system("bsub < run/job_merger_7d8e21_$run_num.sh");
    #system("/bin/rm -f run/job_merger_7d8e21_$run_num.sh");
